...
std::string from_latin1(const std::string& latin1_string) {
	...
	for (auto& latin1_char : latin1_string) {
		if (static_cast<uint8_t>(latin1_char) < 0b10000000) {
			utf8_string.push_back(latin1_char);
		} else {
			utf8_string.push_back(0b11000000 | (static_cast<uint8_t>(latin1_char) >> 6));
			utf8_string.push_back(0b10000000 | (static_cast<uint8_t>(latin1_char) & 0b00111111));
		}
	}
	return utf8_string;
}
...
uint32_t next_character(const std::string& utf8_string, size_t* index) {
	...
	uint8_t current{ static_cast<uint8_t>(utf8_string[*index]) };
	(*index)++;
	...
	if (current >= 0b11111100) {
		if ((current & 0b11111110) == 0b11111100) {
			character = static_cast<uint32_t>(current & 0b00000001);
			remaining = 5;
		}
	} else if (current >= 0b11111000) {
		if ((current & 0b11111100) == 0b11111000) {
			character = static_cast<uint32_t>(current & 0b00000011);
			remaining = 4;
		}
	} else if (current >= 0b11110000) {
		if ((current & 0b11111000) == 0b11110000) {
			character = static_cast<uint32_t>(current & 0b00000111);
			remaining = 3;
		}
	} else if (current >= 0b11100000) {
		if ((current & 0b11110000) == 0b11100000) {
			character = static_cast<uint32_t>(current & 0b00001111);
			remaining = 2;
		}
	} else if (current >= 0b11000000) {
		if ((current & 0b11100000) == 0b11000000) {
			character = static_cast<uint32_t>(current & 0b00011111);
			remaining = 1;
		}
	}
