...
void shader::load_from_source(std::string_view vertex_source, std::string_view fragment_source) {
	...
	info("graphics", "Attributes: {}", attributes);
	...
	for (int location{ 0 }; location < (int)attributes.size(); location++) {
		CHECK_GL_ERROR(glBindAttribLocation(gl_shader.id, location, attributes[location].c_str()));
	}
	...
	if (status == GL_FALSE) {
		error("graphics", "Failed to validate shader program with id {}", gl_shader.id);
	}
...
shader::shader(std::string_view vertex, std::string_view fragment) {
	message("graphics", "Loading shader from source");
...
shader_variable::shader_variable(int shader_id, std::string_view name) : shader_id{ shader_id } {
	CHECK_GL_ERROR(location = glGetUniformLocation(shaders[shader_id].id, name.data()));
	ASSERT(location != -1);
}
