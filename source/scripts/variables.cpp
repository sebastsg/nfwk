...
bool variable::compare(std::string_view right, variable_comparison comparison_operator) const {
...
void variable::modify(const std::string& right, variable_operator modify_operator) {
	...
	case variable_operator::add:
		...
		break;
	default:
		warning(scripts::log, "Cannot run operator {} on strings. String: {}", enum_string(modify_operator), name);
		break;
	}
}
...
void variable::modify(int right, variable_operator modify_operator) {
	...
	case variable_operator::divide:
		if (right != 0) {
			...
			warning(scripts::log, "Prevented dividing {} by 0. Setting the result as 0 to minimize errors.", name);
			std::get<int>(value) = 0;
		}
		break;
	}
...
void variable::modify(float right, variable_operator modify_operator) {
	...
	case variable_operator::divide:
		if (right != 0.0f) {
			...
			warning(scripts::log, "Prevented division of {} by 0.0. Setting the value to 0.0 to minimize errors.", name);
...
void variable::write(io_stream& stream) const {
	stream.write_int32(type);
	stream.write_string(name);
	switch (type) {
	case variable_type::string:
		stream.write_string(std::get<std::string>(value));
		break;
	case variable_type::integer:
		stream.write_int32(std::get<int>(value));
		break;
	case variable_type::boolean:
		stream.write_bool(static_cast<bool>(std::get<int>(value)));
		break;
	case variable_type::floating:
		stream.write_float32(std::get<float>(value));
		break;
	}
...
void variable::read(io_stream& stream) {
	type = stream.read_int32<variable_type>();
	...
	case variable_type::string:
		...
		break;
	case variable_type::integer:
		value = stream.read_int32<int>();
		break;
	case variable_type::boolean:
		value = static_cast<int>(stream.read_bool());
		break;
	case variable_type::floating:
		value = stream.read_float32<float>();
		break;
	}
...
variable* variable_scope::find(std::string_view name) {
...
void variable_scope::add(variable variable) {
	if (!find(variable.name)) {
		...
		warning(scripts::log, "Variable {} already exists.", variable.name);
	}
}

void variable_scope::remove(std::string_view name) {
...
void variable_scope::write(io_stream& stream) const {
	...
	stream.write_size(variables.size());
	for (const auto& variable : variables) {
...
variable* variable_registry::find(std::optional<int> scope_id, std::string_view name) {
...
void variable_registry::read(io_stream& stream) {
	...
	for (std::int32_t i{ 0 }; i < count; i++) {
		scopes.emplace_back(std::make_shared<variable_scope>(stream));
	}
}

std::shared_ptr<variable_scope> variable_registry::find_scope(std::optional<int> scope_id) const {
...
std::ostream& operator<<(std::ostream& out, nfwk::script::variable_comparison type) {
	switch (type) {
	using enum nfwk::script::variable_comparison;
	case equal: return out << "= Equals";
	case not_equal: return out << "!= Not Equals";
	case greater_than: return out << "> Greater Than";
	case less_than: return out << "< Less Than";
	case equal_or_greater_than: return out << ">= Equal or Greater Than";
	case equal_or_less_than: return out << "<= Equal or Less Than";
	}
}

std::ostream& operator<<(std::ostream& out, nfwk::script::variable_type type) {
	switch (type) {
	using enum nfwk::script::variable_type;
	case string: return out << "String";
	case integer: return out << "Integer";
	case boolean: return out << "Boolean";
	case floating: return out << "Float";
	}
}

std::ostream& operator<<(std::ostream& out, nfwk::script::variable_operator type) {
	switch (type) {
	using enum nfwk::script::variable_operator;
	case set: return out << "Set";
	case negate: return out << "Negate";
	case add: return out << "Add";
	case multiply: return out << "Multiply";
	case divide: return out << "Divide";
	}
}
