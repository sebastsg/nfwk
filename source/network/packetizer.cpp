...
void packetizer::start(io_stream& stream) {
	stream.write_int<magic_type>(magic);
	stream.write_int<body_size_type>(0); // offset for the size
}
...
void packetizer::end(io_stream& stream) {
	...
	const auto size = stream.write_index() - header_size;
	stream.set_write_index(sizeof(magic_type));
	stream.write_int<body_size_type>(size);
...
void packetizer::write(const void* data, std::size_t size) {
...
io_stream packetizer::next() {
	...
	if (stream.peek<magic_type>() != magic) {
		warning(network::log, "Skipping magic... {} != {}", stream.peek<magic_type>(), magic);
