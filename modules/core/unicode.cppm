export module nfwk.core:unicode;

import std.core;

export namespace nfwk::unicode {

const std::uint32_t replacement_character{ 0xFFFD };
const std::uint32_t byte_order_mark{ 0xFEFF };
const std::uint32_t byte_order_mark_swapped{ 0xFFFE };

}

export namespace nfwk::utf8 {
...
std::uint32_t next_character(const std::string& utf8_string, std::size_t* index) {
	if (*index >= utf8_string.size()) {
		return unicode::replacement_character;
	}
	auto current = static_cast<std::uint8_t>(utf8_string[*index]);
	(*index)++;
