export module nfwk.core:math.vector2;
...
struct vector2 {
	...
	constexpr vector2<T> operator+(const vector2<T>& that) const {
	...
	void operator+=(const vector2<T>& that) {
		x += that.x;
		y += that.y;
	}
	...
	constexpr bool operator>(const vector2<T>& that) const {
		return x > that.x && y > that.y;
	}
	...
	constexpr T distance_to(const vector2<T>& that) const {
		const T dx{ x - that.x };
		const T dy{ y - that.y };
	...
	constexpr T dot(const vector2<T>& that) const {
		return x * that.x + y * that.y;
	}
...
using vector2b = vector2<std::int8_t>;
