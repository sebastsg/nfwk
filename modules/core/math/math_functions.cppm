export module nfwk.core:math.functions;
...
T value_between(T value, U minimum, U maximum) {
	return std::max(std::min(value, static_cast<T>(maximum)), static_cast<T>(minimum));
}
