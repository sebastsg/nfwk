...
export module nfwk.graphics:assimp_importer;
...
import :model_data;
import :vertex;
import :vertex_array_data;

namespace nfwk {

glm::mat4 ai_mat4_to_glm_mat4(const aiMatrix4x4& ai_matrix) {
...
public:
	...
	assimp_importer(const std::string& input, model_import_options options) : options(options) {
		const int flags{
		...
		scene = static_cast<const aiScene*>(importer.ReadFile(input.string(), flags));
		if (!scene || !scene->mRootNode) {
			warning("graphics", "No scene. Error loading model \"{}\". Error: {}", input, importer.GetErrorString());
			return;
		}
		if (scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE) {
			warning("graphics", "Scene flags incomplete. Error loading model \"{}\". Error: {}", input, importer.GetErrorString());
		}
...
private:
	...
	void load_animations() {
		for (unsigned int a{ 0 }; a < scene->mNumAnimations; a++) {
			...
			for (unsigned int c{ 0 }; c < scene->mAnimations[a]->mNumChannels; c++) {
				...
				skeletal_animation_channel node;
				...
				for (size_t n{ 0 }; n < model.nodes.size(); n++) {
					if (model.nodes[n].name == animation_node_name) {
						/*info("graphics", "[b]Channel #" << c << "[/b]: " << animation_node_name << " uses bone " << node.bone << " as node " << n
				...
				if (!found_node) {
					warning("graphics", "[b]Channel #{}[/b] {} has no matching node. Ignoring.", c, animation_node_name);
				}
			}
		}
	}
	...
	void load_mesh(aiMesh* mesh) {
		...
		for (int b{ 0 }; b < static_cast<int>(mesh->mNumBones); b++) {
			const std::string bone_name{ mesh->mBones[b]->mName.C_Str() };
			...
			for (unsigned int w{ 0 }; w < mesh->mBones[b]->mNumWeights; w++) {
				...
				if (weight.mVertexId >= model.shape.vertices.size()) {
					warning("graphics", "Vertex {} does not exist. Cannot add weight {}", weight.mVertexId, weight.mWeight);
					continue;
				}
				...
				if (vertex.weights.x < 0.0001f) {
					...
					//warning("graphics", "Discarding weight #0 = " << vertex.weights.x << " for vertex " << weight.mVertexId);
					vertex.weights.x = weight.mWeight;
					vertex.bones.x = b;
				} else if (weight.mWeight > vertex.weights.y) {
					//warning("graphics", "Discarding weight #1 = " << vertex.weights.y << " for vertex " << weight.mVertexId);
					vertex.weights.y = weight.mWeight;
					vertex.bones.y = b;
				} else if (weight.mWeight > vertex.weights.z) {
					//warning("graphics", "Discarding weight #2 = " << vertex.weights.z << " for vertex " << weight.mVertexId);
					vertex.weights.z = weight.mWeight;
					vertex.bones.z = b;
				} else if (weight.mWeight > vertex.weights.w) {
					//warning("graphics", "Discarding weight #3 = " << vertex.weights.w << " for vertex " << weight.mVertexId);
					vertex.weights.w = weight.mWeight;
					vertex.bones.w = b;
				} else if (weight.mWeight > vertex.weights_extra.x) {
					//warning("graphics", "Discarding weight #4 = " << vertex.weights_extra.x<< " for vertex " << weight.mVertexId);
					vertex.weights_extra.x = weight.mWeight;
					vertex.bones_extra.x = b;
				} else if (weight.mWeight > vertex.weights_extra.y) {
					//warning("graphics", "Discarding weight #5 = " << vertex.weights_extra.y << " for vertex " << weight.mVertexId);
					vertex.weights_extra.y = weight.mWeight;
					vertex.bones_extra.y = b;
				} else {
					//warning("graphics", "Discarding weight #6 = " << weight.mWeight << " for vertex " << weight.mVertexId);
				}
			}
		}
		if (mesh->mNumBones == 0 && options.bones.create_default) {
			//info("graphics", "No bones. Creating default bone: {}", options.bones.bone_name);
	...
	void load_node(aiNode* node) {
		/*message("graphics", std::string(node_depth, '.') << model.nodes.size() << " [b]" << node->mName.C_Str() << "[/b]: "
		...
		for (unsigned int i{ 0 }; i < node->mNumChildren; i++) {
			model.nodes[index].children.push_back(static_cast<int>(model.nodes.size()));
...
void convert_model(const std::filesystem::path& source, const std::filesystem::path& destination, model_conversion_options options) {
	if (options.exporter) {
