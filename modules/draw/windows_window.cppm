...
export module nfwk.draw:window;
...
bool create_window_class(WNDPROC procedure, std::string_view name) {
	...
	if (RegisterClass(&window)) {
		...
		error("graphics", "Failed to register window class");
		return false;
	}
}

vector2i calculate_actual_window_size(int width, int height, DWORD style) {
	RECT area{ 0, 0, width, height };
	AdjustWindowRect(&area, style, false);
	return { area.right - area.left, area.bottom - area.top };
}

vector4i calculate_maximized_window_rectangle() {
	RECT area{};
	SystemParametersInfo(SPI_GETWORKAREA, 0, &area, 0);
	return { area.left, area.top, area.right, area.bottom };
}

HWND create_window(std::string_view name, std::string_view type, int width, int height, bool maximized) {
	message("graphics", "Creating window: {}. Class: {}", name, type);
	const HINSTANCE instance{ windows::get_current_instance() };
	...
	if (maximized) {
		...
		const auto& xywh = calculate_maximized_window_rectangle();
		const auto x = xywh.x;
		const auto y = xywh.y;
		const auto w = xywh.z;
		const auto h = xywh.w;
...
void destroy_window(HWND window_handle, HDC device_context, windows_gl_context& context) {
	if (window_handle) {
		message("graphics", "Destroying window");
...
public:
	...
	window(std::string_view title, int width, int height, int samples) {
		...
		create_arb_window(title, width, height, samples, false);
	}

	window(std::string_view title, int width, int height) {
		set_clear_color(0.1f);
		set_swap_interval(swap_interval::sync);
		create_default_window(title, width, height, false);
	}

	window(std::string_view title, int samples) {
		set_clear_color(0.1f);
		set_swap_interval(swap_interval::sync);
		create_arb_window(title, 4, 4, samples, true);
	}

	window(std::string_view title) {
		set_clear_color(0.1f);
		set_swap_interval(swap_interval::sync);
		create_default_window(title, 4, 4, true);
	}
	...
	void swap() override {
		if (!SwapBuffers(device_context)) {
			warning("graphics", "HDC: {}\nHWND: {}\nError: {}", device_context, window_handle, GetLastError());
		}
	}
	...
	void set_icon_from_resource(int resource_id) {
		if (const HICON icon{ LoadIcon(platform::windows::current_instance(), MAKEINTRESOURCE(resource_id)) }) {
			...
			warning("graphics", "Failed to load icon resource {}", resource_id);
		}
	}
#endif
...
private:
	...
	void create_default_window(std::string_view title, int width, int height, bool maximized) {
		create_window_class(process_window_messages, "Main");
		window_handle = create_window(title, "Main", width, height, maximized);
		if (!window_handle) {
			error("graphics", "Failed to create window");
			return;
		}
	...
	void create_arb_window(std::string_view title, int width, int height, int samples, bool maximized) {
		if (samples > 0) {
			...
			if (!default_window_handle) {
				error("graphics", "Failed to create default window");
				return;
			}
	...
	void show(bool maximized) {
		int show_command{ windows::get_show_command() };
	...
	windows_gl_context render_context;
...
LRESULT WINAPI process_window_messages(HWND window_handle, UINT message, WPARAM w_param, LPARAM l_param) {
	auto active_window = reinterpret_cast<nfwk::window*>(GetWindowLongPtr(window_handle, 0));
	if (!active_window) {
